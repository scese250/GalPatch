...
std::tuple<std::vector<std::wstring>, std::vector<std::wstring>> KrkrPatcher::ListPatches()
{
    ...
    for (int num = PATCH_COUNT; num > 0; --num)
    {
        ...
        if (const auto patchDir = patchPrefix + L"\\"; GetFileAttributesW(patchDir.c_str()) != INVALID_FILE_ATTRIBUTES && (GetFileAttributesW(patchDir.c_str()) & FILE_ATTRIBUTE_DIRECTORY))
            patchDirs.emplace_back(patchDir);

        if (const auto patchArc = patchPrefix + L".xp3"; GetFileAttributesW(patchArc.c_str()) != INVALID_FILE_ATTRIBUTES && !(GetFileAttributesW(patchArc.c_str()) & FILE_ATTRIBUTE_DIRECTORY))
